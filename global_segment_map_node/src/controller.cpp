...
Controller::Controller(ros::NodeHandle* node_handle_private) :
  ...
  integrator_config.default_truncation_distance = map_config_.voxel_size * 2;
  ...
  mesh_integrator_.reset(new voxblox::MeshLabelIntegrator(
                                 mesh_config_, map_->getTsdfLayerPtr(),
...
void Controller::AdvertiseGenerateMeshService(
  ...
  *generate_mesh_srv = node_handle_private_->
                      advertiseService("generate_mesh",
...
void Controller::AdvertiseExtractSegmentsService(
  ...
  *extract_segments_srv = node_handle_private_->
                  advertiseService("extract_segments",
                                   &Controller::ExtractSegmentsCallback, this);
}
...
void Controller::SegmentPointCloudCallback(
  ...
  if (segment_point_cloud_msg->header.stamp != last_segment_msg_timestamp_) {
    ...
    Method method_ = Method::kMerged;
    bool discard = false;
    if (method_ == Method::kMergedDiscard) {
      discard = true;
    }

    for (const auto& segment: segments_to_integrate_) {
      integrator_->integratePointCloudMerged(segment->T_G_C_,
                                             segment->points_C_,
                                             segment->colors_,
                                             segment->labels_,
                                             discard);
    ...
    ROS_INFO("Finished integrating pointclouds in %f seconds.", (end - start).toSec());

    ROS_INFO("Clearing candidates and memory.");
    start = ros::WallTime::now();

    segment_label_candidates.clear();
    for (auto segment_it = segments_to_integrate_.begin(); segment_it != segments_to_integrate_.end(); ++segment_it) {
      delete *segment_it;
    }
...
//  std::string camera_frame_id = "/camera_depth_optical_frame";
  ...
  if (lookupTransform(camera_frame_id, world_frame_id,
    ...
    ros::WallTime start = ros::WallTime::now();
    integrator_->computeSegmentLabelCandidates(segment, &segment_label_candidates);
...
bool Controller::GenerateMeshCallback(
  ...
  if (clear_mesh) {
    ...
    bool updated = false;
    mesh_integrator_->generateMeshForUpdatedBlocks(clear_updated_flag, &updated);
  }

  // Publish MarkerArray with mesh.
  visualization_msgs::MarkerArray marker_array;
  marker_array.markers.resize(1);
  voxblox::fillMarkerWithMesh(mesh_layer_,
      voxblox::ColorMode::kColor, &marker_array.markers[0]);
  marker_array.markers[0].header.frame_id = "world";
  mesh_pub_->publish(marker_array);

  // Store mesh to file.
  std::string mesh_filename = "voxblox_gsm_mesh.ply";

  if (!mesh_filename.empty()) {
    bool success = voxblox::outputMeshLayerAsPly(mesh_filename, *mesh_layer_);
    if (success) {
      ROS_INFO("Output file as PLY: %s", mesh_filename.c_str());
    } else {
...
void Controller::updateMeshEvent(const ros::TimerEvent& e) {
  ...
  visualization_msgs::MarkerArray marker_array;
  marker_array.markers.resize(1);
  fillMarkerWithMesh(mesh_layer_,
      voxblox::ColorMode::kColor, &marker_array.markers[0]);
  marker_array.markers[0].header.frame_id = "world";
  mesh_pub_->publish(marker_array);
}

bool Controller::ExtractSegmentsCallback(
    std_srvs::Empty::Request& request,
    std_srvs::Empty::Response& response) {
  ...
  // too small to result in any polygon. Find a fix, maybe with a size threshold.

  for (auto& label: labels) {
    voxblox::Layer<voxblox::TsdfVoxel> tsdf_layer(map_config_.voxel_size,
        map_config_.voxels_per_side);
    voxblox::Layer<voxblox::LabelVoxel> label_layer(map_config_.voxel_size,
        map_config_.voxels_per_side);
    ...
    std::string mesh_filename = "segments/voxblox_gsm_mesh_label_"
...
void Controller::extractSegmentLayers(voxblox::Label label,
