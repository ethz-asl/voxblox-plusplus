...
TEST(ICPTest, Registration) {
  ...
  std::string reference_cloud_filename = "segment.ply";
  int iterations = 10;
  pcl::console::TicToc time;
  time.tic();
  ASSERT_GE(pcl::io::loadPLYFile<pcl::PointXYZRGB>(reference_cloud_filename,
                                                   *cloud_in),
            0);

  std::cout << "\nLoaded file " << reference_cloud_filename << " ("
            << cloud_in->size() << " points) in " << time.toc() << " ms\n"
            << std::endl;
  ...
  *cloud_tr = *cloud_icp;  // We backup cloud_icp into cloud_tr for later use
  ...
  pcl::visualization::PointCloudColorHandlerCustom<PointType> cloud_in_color_h(
  ...
  pcl::visualization::PointCloudColorHandlerCustom<PointType> cloud_tr_color_h(
      cloud_tr, 20, 180, 20);
  viewer.addPointCloud(cloud_tr, cloud_tr_color_h, "cloud_tr_v1", v1);

  // ICP aligned point cloud is red
  pcl::visualization::PointCloudColorHandlerCustom<PointType> cloud_icp_color_h(
      cloud_icp, 180, 20, 20);
